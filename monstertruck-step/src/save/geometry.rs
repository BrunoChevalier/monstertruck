...
impl DisplayByStep for Processor<TrimmedCurve<UnitCircle<Point3>>, Matrix4> {
    fn fmt(&self, idx: usize, f: &mut Formatter<'_>) -> Result {
        ...
        let axis = VectorAsDirection(safe_normalize(Vector3::new(
            transform[2][0],
            transform[2][1],
            transform[2][2],
        )));
        let r0 = transform[0].magnitude();
        let r1 = transform[1].magnitude();
        let ref_direction = VectorAsDirection(safe_div(
            Vector3::new(transform[0][0], transform[0][1], transform[0][2]),
            r0,
        ));
        if r0.near(&r1) {
            let r = FloatDisplay(r0);
...
impl DisplayByStep for Processor<TrimmedCurve<UnitHyperbola<Point3>>, Matrix4> {
    fn fmt(&self, idx: usize, f: &mut Formatter<'_>) -> Result {
        ...
        let axis_raw = VectorAsDirection(safe_normalize(Vector3::new(
            transform[2][0],
            transform[2][1],
            transform[2][2],
        )));
        ...
        let r0 = transform[0].magnitude();
        let r1 = transform[1].magnitude();
        let ref_direction_raw = VectorAsDirection(safe_div(
            Vector3::new(transform[0][0], transform[0][1], transform[0][2]),
            r0,
        ));
        let ref_direction = StepDisplay::new(ref_direction_raw, ref_direction_idx);

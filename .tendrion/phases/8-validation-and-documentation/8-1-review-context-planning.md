# Review Context: Plan 8-1 (Planning Review, Round 3 of 3)

## Plan Under Review

**Plan ID:** 8-1
**Plan Path:** .tendrion/phases/8-validation-and-documentation/8-1-PLAN.md
...
|---------|------|-----------|
| 8-2 | 2 | Update FILLET_IMPLEMENTATION_PLAN.md to accurately reflect the final v0.3.0 status. |
...
**Rationale:** `B1` must be fixed before execution. The plan is structurally valid (`td-tools` passed), its wave/dependency alignment with sibling plan `8-2` is coherent, and it cleanly covers the `TOPO-03` side of Phase 8, but the corrupted-shell test is still specified in a way that will not trigger the planned assertion under the plan's own closed-shell-only Euler rule.

## Findings

### Blockers

#### B1: The planned corruption test bypasses the closed-shell Euler assertion [confidence: 94]
- **Confidence:** 94
- **File:** `.tendrion/phases/8-validation-and-documentation/8-1-PLAN.md:145`
- **Issue:** Task 3 says to corrupt a valid shell by removing a face, while Task 1 explicitly makes `euler_poincare_check` return `true` unless `shell.shell_condition() == ShellCondition::Closed`. The topology docs show that an open but compatibly oriented shell is classified as `ShellCondition::Oriented`, so removing a face from a closed box turns it into the exact kind of shell that the Euler check skips.
- **Impact:** `debug_assert_fires_on_corrupted_shell` can pass through `debug_assert_topology` without panicking, which means the plan does not reliably satisfy the locked requirement to prove the assertion fires on corrupted topology.
- **Suggested fix:** Change the corruption strategy so the shell remains `ShellCondition::Closed` while violating `V - E + F == 2`, or explicitly make the test an orientation-corruption case that is expected to fail the orientation assertion instead of the Euler assertion.

### Suggestions

#### S1: Make the closed-box fixture explicit in Task 3 [confidence: 90]
- **Confidence:** 90
- **File:** `.tendrion/phases/8-validation-and-documentation/8-1-PLAN.md:138`
- **Issue:** Task 3 still mentions `build_box_shell()` from `tests.rs`, but the helper with that name builds a 4-face open shell, not the 6-face closed shell required for the planned Euler checks. The repository already has a closed-cube pattern in `build_6face_box()`.
- **Impact:** The task is still executable because it also allows inline replication, but the current wording leaves room for the executor to choose the wrong fixture and debug a false failure.
- **Suggested fix:** Replace the `build_box_shell()` reference with the `build_6face_box()` pattern, or make the 6-face inline construction mandatory.
...
Plan `8-1` is structurally sound and now matches the Phase 8 topology scope well, with `DOC-01` cleanly left to sibling plan `8-2`. The remaining blocker is isolated to the corruption-test design: removing a face makes the shell open/oriented, so it no longer exercises the closed-shell Euler assertion the plan defines. Once that test case is rewritten to trip a real invariant failure, the plan should be ready for execution.
```
